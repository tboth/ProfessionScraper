...
parser.add_argument('-n', '--number', help="URL number part", required=True)
...
url = "https://www.profession.hu/allasok/{}/1,0,{}".format(args['url_part'], args['number'])
...
#===> megfigyelni hogy hogyan valtozik az URL es az szerint beallitani a valtozo adatokat (kategoria, oldalszam)
    url = "https://www.profession.hu/allasok/{}/{},0,{}".format(args['url_part'],i,args['number'])
    html_page = requests.get(url, headers=headers)
    soup = BeautifulSoup(html_page.content, 'lxml')
